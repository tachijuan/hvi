...
| Hardware line shift | `scr_open_row()` / `scr_del_rows()` | `o`/`O` and every linewise delete (`dd`, `ndd`, `dj`, `dG`, … and their `.` repeats) on families with a hardware scroll (v2.9, issue #8) — the rows below the edit shift in place and only rows exposed at the bottom are painted |
| Cursor row to bottom | `scr_redraw_from_cur()` | `J`, `p`, `P`, `u`, `dw`/`cw`/etc. (when not single-row), insert-mode Enter/BS-over-newline/Ctrl-W-across-newline, dot-repeat of insert/join — content above cursor unchanged; also the fallback for the hardware-shift tier |
...
**Hardware line shift (v2.9, issue #8):** `term_shift_up()`/`term_shift_dn()` shift the text rows below screen row `ts_row` by one, leaving the status row untouched. The ANSI build narrows the DECSTBM scroll region to `[ts_row+1, rows-1]`, indexes, and restores the full region (pure VT100 — no VT102 `IL`/`DL` dependence); the ILDL families pair their existing insert/delete-line codes, and `term_scroll_up/dn()` become the `ts_row = 0` wrappers. `scr_open_row()` (o/O) shifts down — the exposed blank row *is* the new empty line, so nothing is drawn; `o` on the bottom row lands in the `scr_update_after_move()` smart scroll. `scr_del_rows()` (linewise `d`) shifts up `sdr_n` times and repaints only the exposed bottom rows; `apply_op` counts the doomed range's visual rows *before* the delete (capped at a screenful). Fallbacks to the ordinary repaint: range starting above the viewport, taller than the text area, reaching the bottom row, an emptied buffer, or a viewport move. Families without a hardware scroll compile the tier away (`scr_open_row()` is a `scr_adj()` macro; the `apply_op` hook is `#ifdef`'d out).
