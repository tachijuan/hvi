...
| Hardware line shift | `scr_open_row()` / `scr_del_rows()` / `scr_ins_rows()` / `scr_join_row()` | `o`/`O`, every linewise delete (`dd`, `ndd`, `dj`, `dG`, … and their `.` repeats), insert-mode Enter and line-joining BS, linewise `p`/`P`, and `u` of linewise-shaped records, on families with a hardware scroll (v2.9 issue #8, v2.10 issue #11) — the rows below the edit shift in place and only the rows the edit changed are painted |
| Cursor row to bottom | `scr_redraw_from_cur()` | `J`, `dw`/`cw`/etc. (when not single-row), insert-mode Ctrl-W-across-newline, dot-repeat of insert/join, non-linewise `p`/`P`/`u` — content above cursor unchanged; also the fallback for the hardware-shift tier |
...
**Hardware line shift (v2.9 issue #8, extended v2.10 issue #11):** `term_shift_up()`/`term_shift_dn()` shift the text rows below screen row `ts_row` by `ts_n`, leaving the status row untouched — the count is batched so a multi-row shift costs one region set/reset (ANSI) or one cursor round trip (ILDL), not one per row. The ANSI build narrows the DECSTBM scroll region to `[ts_row+1, rows-1]`, indexes `ts_n` times, and restores the full region (pure VT100 — no VT102 `IL`/`DL` dependence); the ILDL families batch their existing insert/delete-line codes, and `term_scroll_up/dn()` become the `ts_row = 0, ts_n = 1` wrappers.

The screen tier: `scr_ins_rows()` shifts down and paints only the inserted rows for any linewise-shaped insert — a line start, text ending `'\n'`, cursor at the start, no wrapped line (verified by `scr_count_rows()`, the capped visual-row counter shared with `apply_op` and the undo gates). It serves undo of a linewise delete, the linewise `p`/`P` put, and — through the `scr_open_row()` wrapper, which first handles a moved viewport with the `scr_update_after_move()` smart scroll (safe for the single-`'\n'` case: rows below a 1-byte slide hold unchanged text) — `o`/`O` and insert-mode Enter. An Enter that splits a 1-vrow line reuses the same path (the shifted-in row is painted with the tail; the head row is repainted above). `scr_del_rows()` (linewise `d`, undo of a linewise insert) shifts up `sdr_n` rows counted *before* the delete and repaints only the exposed bottom rows; `scr_join_row()` (insert-mode BS over a line end, gated on both lines and the join each fitting one vrow) repaints the joined row and delegates the vanished row to `scr_del_rows()`. Status handling: `scr_ins_rows()`/`scr_del_rows()` paint no status themselves — normal-mode callers (`apply_op`, undo, put) follow with `status_show()`, while the insert-mode paths (o/CR/BS) leave the `-- INSERT --` indicator alone. Fallbacks to the ordinary repaint: range starting above the viewport, taller than the text area, reaching the bottom row, a wrapped line, an emptied buffer, a failed insert, or a viewport move. Families without a hardware scroll compile the tier away (`scr_open_row()` is a `scr_adj()` macro; the `apply_op` hook and the edit.c gates are `#ifdef`'d out).
